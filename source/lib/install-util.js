...
function getInstallInstructions(data /* :Object */) /* :string */ {
	...
	if (data.name.indexOf(prefix) === 0) {
		...
		if (data.filenamesForPackageFiles.package) {
			...
			parts.push(
				`${npmLink}<ul>` +
					...
					(commands.length
						? `\n<li>Executable${
								commands.length === 1 ? '' : 's'
						  }: <code>${commands.join('</code>, <code>')}</code></li>`
						: '') +
					(flag === '--save'
			...
			// Browser
			if (data.browsers) {
				const browserifyLink = getLink({
					text: '<h3>Browserify</h3>',
				...
				parts.push(
					[
						...
						`<li>Module: <code>require('${data.name}')</code></li>`,
						`<li>CDN URL: <code>//wzrd.in/bundle/${data.name}@${
							data.version
						}</code></li></ul>`
					].join('\n')
				)

				const enderLink = getLink({
					text: '<h3>Ender</h3>',
					url: 'http://enderjs.com',
					title: 'Ender is a full featured package manager for your browser'
				})
				parts.push(
					[
						`${enderLink}<ul>`,
		...
		if (data.editions) {
			...
			const linksArray = [
				...
				},
				{
					...
					title:
						'A polyfill that emulates missing ECMAScript environment features'
				},
				{
					text: 'Babel',
